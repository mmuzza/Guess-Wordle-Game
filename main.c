...
//
//  Author: Muhammad Muzzammil, 11/21/2022
//  System: Replit
//
...
void printingWords(char secretWord[], char printWord[], int num){
  ...
  // Following for loop makesall copies that are needed
  for(int i = 0; i < 5; i++){
    printCopy[i] = printWord[i];
    printCopy2[i] = printWord[i];
    secretCopy[i] = secretWord[i];
  }


  // prints each word count and proper spacing
  printf("    %d. ", num);

  // Capitalize all letters that have exact match
  // print them as we go
  for(int i = 0; i < 5; i++){
    if(secretWord[i] == printWord[i]){
      printf("%c ", toupper(printWord[ i]));
      printCopy[i] = toupper(printWord[ i]);
      printCopy2[i] = toupper(printWord[ i]);
      secretCopy[i] = ' ';
    }
    else{
      //printCopy[i] = ' ';
      printf("%c ", (printWord[ i]));
    }
  }
  printf("\n");


  // Whatever is not capital and matches will get a star
  for(int i = 0; i < 5; i++){
    for(int k = 0; k < 5; k++){
      if(printCopy[i] == secretCopy[k]){
        starWord[i] = '*';
        secretCopy[k] = ' ';
      }
    }
        ...
        char secretWord[])              // The word to be guessed
{
  ...
  int low, mid, high;
   ...
   {
     mid=(low+high)/2;
     printingWords(secretWord, allWords[mid].word, num);
     
     if (strcmp(secretWord, allWords[mid].word)==0)
     {
       
       break;
     }
     else if(strcmp(secretWord, allWords[mid].word)>0)
    {
      high=high;
      low=mid+1;
    } 
